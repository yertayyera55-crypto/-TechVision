...
def run() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        expect(page.get_by_text("Приёмка подтверждается подписанной накладной в течение 1 рабочего дня после поставки", exact=True)).to_be_visible()
        expect(page.get_by_text("Уступка денежного требования разрешена без отдельного согласия покупателя после письменного уведомления", exact=True)).to_be_visible()
        expect(page.get_by_text("Не хватает данных:", exact=True)).to_have_count(0)
        ...
        expect(page.get_by_text("Дополнительные документы · необязательно сейчас", exact=True)).to_be_visible()
        ...
        expect(page.get_by_text("Нужно заполнить", exact=True)).to_have_count(0)
        expect(page.locator("#acceptanceTerms")).to_have_value("Приёмка подтверждается подписанной накладной в течение 1 рабочего дня после поставки")
        expect(page.locator("#assignmentTerms")).to_have_value("Уступка денежного требования разрешена без отдельного согласия покупателя после письменного уведомления")
        ...
        expect(page.get_by_text("Самый подходящий для этой заявки — Bereke Flow", exact=True)).to_be_visible()
        offer_buttons = page.get_by_role("button", name="Выбрать предложение", exact=True)
        expect(offer_buttons).to_have_count(3)
        ornek_card = page.locator("article").filter(has_text="Örnek Capital")
        expect(ornek_card.get_by_role("button", name="Выбрать предложение", exact=True)).to_be_enabled()
        ornek_card.get_by_role("button", name="Выбрать предложение", exact=True).click()
