...
function AnalysisSummary({ analysis }: { analysis: ContractAnalysisResult }) {
  const rows = [["Покупатель", analysis.network || analysis.buyerName || "Не найден"], ["Сумма", analysis.amount ? `${new Intl.NumberFormat("ru-RU").format(analysis.amount)} ₸` : "Не найдена"], ["Срок оплаты", analysis.paymentDueDate || (analysis.paymentTermDays ? `${analysis.paymentTermDays} дней` : "Не найден")]];
  return <><dl className="mt-3 grid gap-2 sm:grid-cols-3">{rows.map(([label, value]) => <div key={label} className="bg-white/70 px-3 py-2"><dt className="text-[10px] uppercase tracking-wide text-slate-500">{label}</dt><dd className="mt-1 text-xs font-semibold text-ink">{value}</dd></div>)}</dl>{analysis.missingData.length > 0 && <p className="mt-3 text-xs leading-5 text-amber-900"><strong>Нужно уточнить:</strong> {analysis.missingData.join(", ")}</p>}</>;
}
