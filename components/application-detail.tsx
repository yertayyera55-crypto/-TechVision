...
export function ApplicationDetail({ id }: { id: string }) {
  ...
  const acceptOffer = async (selected?: PartnerOffer) => {
    const factoringOffer = selected ? { deliveryId: id, financingPercentage: selected.financingPercentage, financingAmount: selected.financingAmount, financingCost: selected.cost, documentFees: 0, otherFees: 0, taxExpenses: 0, platformFee: 0, netAmount: selected.netAmount, savedDays: selected.termDays, factoringType: selected.factoringType, gracePeriodDays: 20 } : application.factoringOffer;
    if (!factoringOffer) return;
    setTransferring(true);
    await new Promise((resolve) => setTimeout(resolve, 700));
    const monitoring = createDemoMonitoring(id, application.amount, application.paymentDueDate, factoringOffer.gracePeriodDays, factoringOffer.netAmount);
    update({ status: "awaiting_buyer_payment", transferredAt: new Date().toISOString(), dataTransferConsent: true, monitoring, factoringOffer, selectedFactoringType: factoringOffer.factoringType, selectedPartnerOfferId: selected?.id });
    setTransferring(false);
    window.dispatchEvent(new CustomEvent("mm-toast", { detail: "Демопредложение выбрано" }));
  };
  ...
  return <div className="animate-rise">
    ...
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,.75fr)]"><div className="space-y-7">
      ...
      {!application.monitoring && application.factoringOffer && <PreliminaryOffer application={application} loading={transferring} onAccept={(offer) => void acceptOffer(offer)} />}
      {application.monitoring && <section className="border-y border-blue-200 bg-blue-50/70 px-4 py-6 sm:rounded-lg sm:border sm:p-6"><div className="flex items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white text-blue-700 ring-1 ring-blue-200"><Send className="h-5 w-5" /></span><div><p className="eyebrow !text-blue-800 mb-1">Синтетический сценарий</p><h2 className="text-xl font-semibold text-ink">Демопредложение выбрано</h2><p className="mt-2 text-sm leading-6 text-slate-600">В MVP дальнейшее финансирование и оплата показаны условно. В реальном продукте деньги и статусы поступали бы от выбранного финансового партнёра.</p><Link href={`/deals/${id}/monitoring`} className={`${primaryLinkClass} mt-5`}>Перейти в «Сроки оплаты»</Link></div></div></section>}
      ...
      {application.readiness && application.productCategory && <section aria-labelledby="readiness-heading"><div className="mb-4"><p className="eyebrow mb-1">Паспорт сделки</p><h2 id="readiness-heading" className="section-title">Предварительная готовность</h2></div><div className="grid gap-px border border-line bg-line sm:grid-cols-3"><Detail label="Категория" value={getCategoryConfig(application.productCategory).label} /><Detail label="Подтверждённое требование" value={formatCurrency(application.receivable?.confirmedReceivable ?? application.amount)} /><Detail label="Полнота документов" value={`${application.readiness.documentCompleteness}%`} /></div>{application.readiness.missingDocuments.length > 0 && <p className="mt-3 text-xs leading-5 text-amber-900"><strong>Нужно уточнить:</strong> {application.readiness.missingDocuments.join(", ")}. Это предварительная проверка; финальное решение принимает партнёр.</p>}</section>}
...
function Detail({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) { return <div className="border-b border-line px-4 py-4 last:border-0 sm:[&:nth-last-child(-n+2)]:border-b-0 sm:[&:nth-child(odd)]:border-r"><dt className="flex items-center gap-2 text-xs text-slate-500">{icon}{label}</dt><dd className="mt-1.5 text-sm font-semibold text-ink">{value}</dd></div>; }
