...
export function PaymentsMonitoringDashboard() {
  ...
  return <div className="animate-rise"><header className="mb-7 border-b border-line pb-7"><p className="eyebrow mb-2">Информационный раздел</p><h1 className="font-display text-4xl font-medium tracking-tight md:text-5xl">Сроки оплаты</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-muted">После выбора демопредложения покупатель условно перечисляет оплату финансовому партнёру. В MVP статусы синтетические и не редактируются вручную.</p></header>{error && <MonitoringErrorState message={error} onDismiss={clearError} />}<PaymentMonitoringSummary deals={allDeals} /><div className="mt-7"><PaymentMonitoringFilters filter={filter} query={query} sort={sort} onFilter={setFilter} onQuery={setQuery} onSort={setSort} /></div><div className="mt-5"><div className="mb-3 flex items-center justify-between gap-3"><h2 className="text-lg font-semibold">Реестр сделок</h2><p className="text-xs text-slate-500">{hydrated ? `Найдено: ${filtered.length}` : "Загрузка…"}</p></div>{filtered.length ? <><PaymentMonitoringTable deals={filtered} /><div className="grid gap-3 lg:hidden">{filtered.map((deal) => <PaymentMonitoringCard key={deal.id} deal={deal} />)}</div></> : <MonitoringEmptyState onReset={resetFilters} />}</div><p className="mt-6 text-xs leading-5 text-slate-500">При факторинге с регрессом финансовый партнёр может предусмотреть возврат финансирования при длительной просрочке. Это пояснение демо-условия, а не требование возврата.</p></div>;
}
