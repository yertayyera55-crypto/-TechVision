...
export function ApplicationsList() {
  ...
  return <div className="animate-rise">
    ...
    <div className="mb-5 grid gap-3 sm:grid-cols-[1fr_240px]"><label className="relative"><span className="sr-only">Поиск заявок</span><Search className="pointer-events-none absolute left-3.5 top-3.5 h-5 w-5 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="control pl-11" placeholder="Номер, покупатель или накладная" /></label><label><span className="sr-only">Фильтр по статусу</span><select className="control" value={status} onChange={(event) => setStatus(event.target.value as "all" | ApplicationStatus)}><option value="all">Все статусы</option><option value="precheck_passed">Предварительно соответствует условиям</option><option value="awaiting_confirmation">Ожидает подтверждения</option><option value="clarification_required">Нужно уточнение</option><option value="delivery_confirmed">Поставка подтверждена</option><option value="awaiting_buyer_payment">Ожидается оплата покупателя</option><option value="payment_overdue">Оплата просрочена</option><option value="closed">Сделка закрыта</option></select></label></div>
