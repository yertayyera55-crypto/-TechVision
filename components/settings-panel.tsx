...
import { clearDocumentFiles } from "@/lib/document-file-storage";
...
export function SettingsPanel() {
  ...
  const { resetDeals } = usePaymentMonitoring();
  const [settings, setSettings] = useState(defaults);
  const [saving, setSaving] = useState(false);
  const [resetting, setResetting] = useState(false);
  /* eslint-disable react-hooks/set-state-in-effect -- settings hydrate from browser storage after SSR. */
  useEffect(() => { const saved = window.localStorage.getItem(SETTINGS_KEY); if (saved) { try { setSettings(JSON.parse(saved)); } catch (error) { console.error("Не удалось загрузить настройки:", error); } } }, []);
  ...
  const save = async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent("mm-toast", { detail: "Настройки сохранены" }));
    } catch (error) {
  ...
  const reset = async () => {
    ...
    try {
      ...
      console.error("Не удалось сбросить demo-данные:", error);
      window.dispatchEvent(new CustomEvent("mm-toast", { detail: "Не удалось сохранить изменения. Попробуйте ещё раз" }));
    } finally {
      setResetting(false);
    }
